...
function addVanessa() {
  ...
  player.textContent = "👧🏻";
  player.style.left = mapPositions[currentCountry].x / 5 + "px";
  player.style.top = mapPositions[currentCountry].y / 5 - 30 + "px";
...
function openPopup() {
  popupCountry.textContent = "🇪🇺 " + currentCountry;
  popupFrog.textContent = countries[currentCountry].frog;
  ...
  const frogSound = new Audio("sounds/" + countries[currentCountry].sound);
  frogSound.play();
}
...
function updateStats() {
  ...
  visitedCounter.textContent = visitedCountries.length;
}
